...
export const dateHandler = (dateStr: string | undefined, pathname?: string | undefined) => {
  ...
  if (daysDifference === 0) {
    ...
    // Return the full date with time for dates beyond 1 week in the user's timezone 
    const options: Intl.DateTimeFormatOptions = {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
...
export const getTotal = (items: Listing[], agentName: string) => {
  ...
  items.forEach((item: { realtor: { user: { full_name: string } } }) => {
    if (item.realtor.user.full_name === agentName) {
      totalCount++;
    }
  });
